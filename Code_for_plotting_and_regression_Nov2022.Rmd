...
```{r DONE AIC to look at frequency bands}

# read data and compute data for demographic table
data=read.csv('~/Documents/PD_fingerprinting/QPN_demo_compiled.csv', stringsAsFactors = FALSE)


self_id_aic = read.csv('~/Documents/PD_fingerprinting/new_analysis_Nov2022/Delta_AIC_fullband_results.csv', na.strings = 'NaN', stringsAsFactors = FALSE)

data=cbind(data, self_id_aic[,2:7])
...
lm1=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU + delta_removed, data4reg)
lm2=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU +theta_removed, data4reg)
lm3=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU +alpha_removed, data4reg)
lm4=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU +beta_removed, data4reg)
lm5=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU +gamma_removed, data4reg)
lm6=lm(updrs2 ~ Age + Education_coded+ DiseaseDur+ HLU + Hgamma_removed, data4reg)
...
ggsave('~/Documents/PD_fingerprinting/figure/Donut_Freq_spectrum.pdf', device = "pdf", width=2, height=2, dpi=800)
```
...
ggsave('~/Documents/PD_fingerprinting/figure/Donut_Freq_spectrum_MOCA_MCI.pdf', device = "pdf", width=2, height=2, dpi=800)
...
ggsave('~/Documents/PD_fingerprinting/figure/Donut_Freq_spectrum_MOCA_noMCI.pdf', device = "pdf", width=2, height=2, dpi=800)
...
t.test(data_new$HLU[data2$Group == 'Control'], data_new$HLU[data2$Group == 'Parkinson'])
t.test(data_new$EOG[data2$Group == 'Control'], data_new$EOG[data2$Group == 'Parkinson'])
t.test(data_new$ECG[data2$Group == 'Control'], data_new$ECG[data2$Group == 'Parkinson'])
...
cor.test(data2$long_auto, data2$recording_length)

lm1= lm(recording_length~ Group, data2)

lm1= lm(long_auto ~recording_length + Group, data2)

cor.test(data2$long_corss, data2$recording_length)

data_new=data[data2$recording_length >50,]
data2= data2[data2$recording_length >50, ]
data_new$recording_length = data2$recording_length

cor.test(data2$long_auto[data2$recording_length >50], data2$recording_length[data2$recording_length >50])

write.csv(data_new, '~/Documents/Alex_fingerprinting/new_analysis_Nov2022/QPN_demo_compiled.csv')

data2= data2[data2$recording_length >50, ]

cor.test(data2$long_auto, data2$recording_length)
t.test(data2$recording_length[data2$Group == 'Control'], data2$recording_length[data2$Group == 'Parkinson'])

lm1= lm(recording_length~ Group, data2)
summary(lm1)

cor.test(data_new$HLU, data2$recording_length)
cor.test(data_new$EOG, data2$recording_length)
cor.test(data_new$ECG, data2$recording_length)
...
cor.test(data2$long_auto, data2$recording_length)
cor.test(data_new$Age, data2$recording_length)
cor.test(data_new$updrs2, data2$recording_length)
cor.test(data_new$moca2, data2$recording_length)
cor.test(data_new$DiseaseDur, data2$recording_length)
cor.test(data_new$Hoehn.and.Yahr.score, data2$recording_length)
cor.test(data_new$HLU, data2$recording_length)

RVAideMemoire::perm.t.test(data2$long_auto[data2$Group== 'Control'],data2$long_auto[data2$Group== 'Parkinson'], nperm=1000)


data3=read.csv('/Users/jason/Documents/Alex_fingerprinting/new_auto_cross_corr.csv')
cor.test(data3$diff_in_auto_corr, data3$record_length)
cor.test(data3$diff_longVSsame, data3$record_length)


# artifacts across runs 

artifacts=read.csv('~/Documents/Alex_fingerprinting/artifacts_across_session.csv')
t.test(artifacts$HLU[artifacts$recording==1], artifacts$HLU[artifacts$recording==2])
lm1=lm(HLU~ Group*recording, artifacts)
sjPlot::tab_model(lm1)

lm1=lm(HLU~ recording, artifacts)
sjPlot::tab_model(lm1)


lm1=lm(EOG~ Group*recording, artifacts)
sjPlot::tab_model(lm1)

lm1=lm(EOG~ recording, artifacts)
sjPlot::tab_model(lm1)



lm1=lm(ECG~ Group*recording, artifacts)
sjPlot::tab_model(lm1)

lm1=lm(ECG~ recording, artifacts)
sjPlot::tab_model(lm1)


data_length= read.csv('~/Documents/PD_fingerprinting/QPN_ExcludedEpochMeanDistances_forJason_11162022.csv')

lm1= lm(Run1~ Group,data_length )
sjPlot::tab_model(lm1)
lm1= lm(Run1~ Group*Run,data_length )
sjPlot::tab_model(lm1)
...
write.csv(target, '~/Documents/PD_fingerprinting/features_4_fingeprinting_target.csv' )
write.csv(database, '~/Documents/PD_fingerprinting/features_4_fingeprinting_database.csv' )

temp1=read.csv('~/Documents/PD_fingerprinting/features_4_fingeprinting_target.csv' )
